...
for RC in ~/.bashrc ~/.zshrc; do
    ...
    grep -q "alias b="  "$RC" 2>/dev/null || \
        echo "alias b='cd ~/$BUILD_DIR && source build/envsetup.sh && breakfast Spacewar lunch voltage_Spacewar-trunk_staging-user && mka baconlunch voltage_Spacewar-trunk_staging-user && mka bacon brunch Spacewar'" >> "$RC"
    ...
    grep -q "alias build=" "$RC" 2>/dev/null || \
        echo "alias build='cd ~/$BUILD_DIR && source build/envsetup.sh && breakfast Spacewar lunch voltage_Spacewar-trunk_staging-user && mka baconlunch voltage_Spacewar-trunk_staging-user && mka bacon brunch Spacewar'" >> "$RC"
